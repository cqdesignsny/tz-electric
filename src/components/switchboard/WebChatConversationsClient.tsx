...
export default function WebChatConversationsClient({
  conversations,
  activeId,
  active,
  messages,
  ...
  async function postAction(payload: Record<string, unknown>) {
    const res = await fetch('/api/agents/web-chat/conversations', {
      method: 'POST',
    ...
    <div className="grid grid-cols-1 md:grid-cols-[340px_1fr] gap-4">
      ...
      {active ? (
        <div className="rounded-2xl border border-gray-200 dark:border-navy-light/40 bg-white dark:bg-[#0F1C3F] overflow-hidden flex flex-col min-h-[600px]">
          ...
          <ConversationMeta active={active} />
          ...
          <ComposeBox
            ...
            placeholder={
              active.takeover_by_user
                ? 'Type your reply as office staff. (Note: customer-side delivery of office replies in the live web chat is a follow-up; for now this saves to the transcript only.)'
        ...
        <div className="rounded-2xl border border-dashed border-gray-300 dark:border-navy-light/50 p-10 text-center text-sm text-gray-500 dark:text-gray-400 min-h-[600px] flex items-center justify-center">
          {conversations.length === 0
            ? 'No web chat conversations yet. As soon as visitors start chatting with Claire on /claire, threads appear here.'
    ...
    <div className="rounded-2xl border border-gray-200 dark:border-navy-light/40 bg-white dark:bg-[#0F1C3F] overflow-hidden">
      ...
      <ul className="divide-y divide-gray-100 dark:divide-navy-light/40 max-h-[640px] overflow-auto">
        {conversations.map((c) => (
          <li key={c.id}>
            <Link
              href={`/switchboard/web-chat?id=${encodeURIComponent(c.id)}`}
              className={[
...
function ConversationMeta({ active }: { active: AgentConversation }) {
  // Pull a few useful first-touch attribution fields out of the JSONB
  // blob without exploding if they're missing.
  const ft = (active.attribution_first_touch ?? {}) as Record<string, unknown>
  const referrer = typeof ft.referrer === 'string' ? ft.referrer : null
  const landingUrl = typeof ft.landingUrl === 'string' ? ft.landingUrl : null
  const gclid = typeof ft.gclid === 'string' ? ft.gclid : null
  const utmSource = typeof ft.utm_source === 'string' ? ft.utm_source : null
  const utmCampaign = typeof ft.utm_campaign === 'string' ? ft.utm_campaign : null

  const hasMeta = referrer || landingUrl || gclid || utmSource || utmCampaign || active.tz_lead_id

  if (!hasMeta) return null

  return (
    <div className="px-4 py-2.5 border-b border-gray-100 dark:border-navy-light/40 bg-gray-50/60 dark:bg-[#0A1128]/60 text-[11px] text-gray-600 dark:text-gray-400 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1">
      {active.tz_lead_id && (
        <div>
          <span className="font-bold text-emerald-700 dark:text-emerald-300">Lead: </span>
          <Link
            href={`/switchboard/lead-pipeline?id=${encodeURIComponent(active.tz_lead_id)}`}
            className="text-blue dark:text-blue-light underline-offset-2 hover:underline font-mono"
          >
            {active.tz_lead_id.slice(0, 8)}…
          </Link>
        </div>
      )}
      {active.attribution_channel && (
        <div>
      ...
      {gclid && (
        <div className="truncate">
          <span className="font-bold">gclid: </span>
          <span className="font-mono">{gclid.slice(0, 18)}…</span>
        </div>
      )}
      {referrer && (
      ...
      {landingUrl && (
        <div className="truncate sm:col-span-2">
          <span className="font-bold">Landed on: </span>
  ...
  if (messages.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center text-sm text-gray-500 dark:text-gray-400 px-6 py-12 text-center">
        No messages persisted yet for {visitorLabel(active)}.
      </div>
    )
  }
...
function MessageBubble({ message }: { message: AgentMessage }) {
  ...
  if (isToolUse || isToolResult) {
    // Tool calls / results render compact so the office can see the
    // chain of work Claire did without reading raw JSON every time.
    const json = message.tool_input
      ? JSON.stringify(message.tool_input)
      : message.content || ''
    return (
      <details className="text-[11px] font-mono text-gray-500 dark:text-gray-400 px-2 group">
        <summary className="cursor-pointer">
          <span className="uppercase tracking-wider font-bold">{message.role}</span>
          {message.tool_name ? `: ${message.tool_name}` : ''}{' '}
          <span className="text-gray-400 dark:text-gray-500">·{' '}{relativeTime(message.created_at)}</span>
        </summary>
        <pre className="mt-1 whitespace-pre-wrap break-all rounded bg-gray-100 dark:bg-[#070D1F] p-2 max-h-48 overflow-auto">
          {json}
        </pre>
      </details>
    )

...
const CHANNEL_FRAMING: Record<CustomerAgentChannel, string> = {
  ...
  voice: [
    ...
    '   - Did the caller name a SPECIFIC staff member by name (Tyler, Ty, Terry, Molly, Sam, Mike, April)? → call `notify_team_member` with target_name + caller_name + caller_phone + a one-line brief_message. That texts the staff member\'s cell directly AND emails the office group.',
...
export async function buildSystemPrompt(input: BuildSystemPromptInput): Promise<string> {
  ...
  sections.push(
    [
      ...
      '- notify_team_member texts a SPECIFIC office staff member\'s cell directly (and also fires the office group email). Use ONLY when the caller named a specific person — Tyler, Ty, Terry, Molly, Sam, Mike, April, etc. Pass target_name + caller_name + caller_phone + a one-line brief_message. The staff directory + matching rules live in section 3 of the knowledge base. If the name doesn\'t match anyone in the directory, the tool still fires the group email and tells you so.',

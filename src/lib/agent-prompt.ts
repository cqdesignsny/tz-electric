...
export async function buildSystemPrompt(input: BuildSystemPromptInput): Promise<string> {
  ...
  sections.push(
    [
      ...
      '- lookup_existing_project tells you whether THIS caller already has work with us — an active quote/estimate or an upcoming appointment — identified by the number they are calling from (NO arguments). Reach for it the moment a returning caller is following up on existing work, so you can recognize and reference their project and route them to the office for a status update INSTEAD of re-running qualification (see the "Returning Customer Following Up" block above). If it returns found=false, do not guess — take a message with flag_for_office_review. It never returns pricing.',
      ...
      '2. Call lookup_existing_project to confirm what they already have on file, so you can reference it back naturally ("I see your 100-to-200 amp service upgrade — it looks like it\'s on the calendar for Thursday the 11th", or "I see your quote for the service upgrade"). One light clarifying question is fine ("Which project is this — the service upgrade?"); nothing more.',

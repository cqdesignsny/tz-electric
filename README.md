...
└── lib/
    ...
    ├── agent-tools.ts             # buildAgentTools (10 tools) — update_visitor_contact, find_existing_customer,
    │                              # create_lead_with_estimate, lookup_business_hours, lookup_my_appointment,
    │                              # lookup_existing_project (returning-customer recognition), flag_for_office_review,
    │                              # notify_team_member, escalate_emergency, dispatch_after_hours_emergency. Shared by
    │                              # web chat, voice, SMS; auto-wires to Vapi via buildVapiFunctionDefinitions.
    ...
    ├── housecall-pro.ts           # HCP API client + empirical findings. find/create customer, estimates, Job Inbox;
    │                              # getUpcomingJobForCustomer + getActiveEstimatesForCustomer (Claire read-only lookups)
...
- **Call Logs** (`/switchboard/call-logs`) **— LIVE 2026-05-13.** Read-only viewer for Voice Claire (`+15186786153`) inbound calls. Two-pane layout, status filter pills, inline audio playback of the Vapi recording, two-sided transcript (customer + Claire) with collapsible tool-call rows, Vapi call-id debug pill, deep link to any lead Claire booked. **Mark-for-review (2026-06-01):** a ⚑ toggle + optional note on any call flags it for a focused Claire-improvement pass (`tz_call_review_marks`, migration 018), plus a "⚑" filter to see only flagged calls — this is how the David Kloss call was flagged for the session-28 fixes.
...
- **SMS opt-in:** public `/sms-opt-in` page (consent CTA + verbatim opt-in language + STOP/HELP + no-third-party-sharing) for A2P 10DLC carrier verification, linked from footer + sitemap
...
| Anthropic (direct, via Vapi) | Live | Claude Sonnet 4.6 behind Voice Claire (swapped from Haiku 4.5 in session 29, mainly for the reasoning-leak fix). NOTE (session 31): the voice "0 leads" bug was NOT the model — prod ran Sonnet and still misfired 50/52. The real cause was the `create_lead` `qualification` `z.record` schema the Vapi tool path can't serialize; flattened to a plain string in session 31. Model fees billed by Vapi. |
...
7 individual job listing pages at `/careers/[slug]` with SSG:
- Lead Electrician, HVAC Project Manager, HVAC Installer, Estimator, Apprentice, Office Assistant, Warehouse Associate (added session 29)
...
- **Known follow-ups:** publish dates are approximate (the old site did not expose them; editable per post). A Switchboard authoring UI for Tyler (write/publish without code) is a deferred Phase 2. (The reused-hero-image issue was resolved in session 31 — all 20 heroes are unique, 3 generated via the Higgsfield image tool.)
...
- ✓ **Follow-Ups / Callbacks hub (LIVE 2026-05-29).** Live Switchboard page (`/switchboard/follow-ups`, owner/admin/office) listing every open callback Claire flagged (`notify_team_member` + `flag_for_office_review`) over the last 14 days, grouped by who it's for, with click-to-call number, Claire's note, a deep link to the conversation, and a **Mark-done dropdown that records the outcome** (booked / declined / no answer / left message / wrong number / other). Closed items move to a collapsible **"Recently handled"** section with the outcome + who + when (reopenable), so the office can confirm the callback was actually made. It's the always-on version of the 6 PM recap email — anyone can open it to see who still needs a callback without email. New `tz_followup_resolutions` table (migration 016, keyed on the flag message id, presence = done), `src/lib/followups.ts`, `/api/switchboard/follow-ups/resolve`. Additive — never touches Claire or the flag tools.
- ✓ **User management: inline name edit + Disable/Re-enable (LIVE 2026-05-29, commit `f06a466`).** Owners can now fix a user's display name inline (the ✎ next to the name on `/switchboard/users`) and explicitly Disable / Re-enable accounts. Disable is a soft-disable (`setUserRole` → `disabled` stamps `disabled_at` and blocks access) that preserves the account + audit history — no hard delete. New `setUserName` (audit-logged) + `/api/switchboard/users/name`.
...
9. **SMS Claire model wire-up + flip `TWILIO_SMS_ENABLED`.** All outbound SMS (after-hours dispatch texts, `notify_team_member` paging) and inbound SMS Claire are blocked until the A2P 10DLC **campaign** clears carrier review. Status as of 2026-06-10 (session 31): after the 5/29 resubmit was rejected with error 30909 ("CTA could not be verified"), session 31 built a public **`/sms-opt-in`** page (the verifiable opt-in CTA the carrier reviewer needs, showing the consent checkbox + verbatim language at one URL) and **resubmitted** via the Twilio API (deleted the FAILED campaign, re-POSTed with `message_flow` pointing at `/sms-opt-in`). Now **`IN_PROGRESS`**, awaiting carrier review. When it shows `REGISTERED`/`APPROVED`, flip `TWILIO_SMS_ENABLED=true`. The moment it shows `REGISTERED`/`APPROVED`: (a) set `TWILIO_SMS_ENABLED=true` on Vercel — `sendSms` reads it at runtime, re-enabling dispatch + paging with no code change; (b) replace the TODO block in `src/app/api/agents/sms/webhook/route.ts` with a `generateText({...})` call (~30 min). Re-check status with the curl in HANDOFF.md.

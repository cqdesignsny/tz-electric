...
**Last verified:** 2026-05-08, end of session 18. Major batch of Tyler's feedback shipped: HCP lead routing pivoted to a single-estimate Google-style flow, two new public pages (`/stay-cool` billboard landing, `/hvac-maintenance` modular pricing page), floating buttons made less intrusive, agent prompt updated with 9am scheduling rule and stay-in-lane policy, dead newsletter form replaced with real CTAs. All three locations (GitHub / SSD / Dropbox) synced. Run the sanity check at the bottom of this doc to confirm before you start.
...
- [x] ~~HCP routing pivot to single-estimate Google-style flow (2026-05-08, supersedes the previous Job Inbox approach).~~ Tyler showed a Google "Reserve with Google" lead in his Inbox that had the full service info populated, while our `/leads`-POST inbox cards always had an empty "Additional notes" panel. After exhaustive probing across all 35 leads in the account, confirmed HCP's public API does not expose any way to set notes on a `/leads` resource (no top-level `notes`/`additional_notes`/`service_details`/`summary`/`customer_availability`, no `/leads/{id}/notes` sub-endpoint, no PATCH/PUT). However, Google's lead is actually an `/estimates` resource with `lead_source: "Reserve with Google"` and rich `option.notes` — HCP's Inbox UI renders that as "Estimate for X" with the source badge and the option notes surfaced in the Additional notes panel. Pivoted both write paths (web form `/api/leads/submit` and Claire's `create_lead_with_estimate`) to a single-estimate flow with one of HCP's locked-preset lead source values. Web form uses `lead_source: "Lead Form"`, Claire uses `lead_source: "CSR AI"`. Dropped the `createInboxLeadForEstimate` POST entirely — no more duplicate Inbox entries. Verified end-to-end via `scripts/e2e-test-lead.mjs`: 6 of 6 checks pass, estimate appears in Inbox with Lead Form badge and full qualification + attribution dump as Additional notes. The HCP `/lead_sources` endpoint exposes the full 38-value preset whitelist; locked presets (Lead Form, CSR AI, Online Booking, Reserve with Google, etc.) get HCP-recognized integration treatment. See "HCP empirical findings — what works, what doesn't" section below for the full evidence.

...
**Last verified:** 2026-06-10, **session 31** (Cesar, second sitting the same day). Session 31 shipped the REAL fix for the voice lead-booking bug, resubmitted A2P, and finished the blog images. The lead bug: `create_lead_with_estimate`'s `qualification` field was a dynamic-key object (`z.record`) that Vapi's tool-calling path could not serialize, so voice fired empty `{}` args on 50 of 52 calls and booked almost no HCP leads. The Haiku→Sonnet swap (session 29) never fixed it because the model was never the cause (prod is confirmed on Sonnet). Flattened `qualification` to a plain string. A2P: built a verifiable `/sms-opt-in` page and resubmitted via the Twilio API (deleted the FAILED campaign, re-POSTed; now `IN_PROGRESS`, no errors). Blog: replaced the last duplicate hero images so every post is unique. All live in prod (HEAD `bc92cec`; SSD, Dropbox, origin in sync). Session 30 (earlier the same day) shipped the blog migration. See **"⭐ NEXT SESSION — START HERE"** below, then the **SESSION 31 / 30 / 29** detail blocks. All prior history (session 28 and earlier) is preserved underneath.

---

## ⭐ NEXT SESSION — START HERE (picking up after session 31, 2026-06-10)

Session 31 is live in prod and pushed (HEAD `bc92cec`; SSD = Dropbox = origin). It shipped the REAL fix for the voice lead-booking bug (flattened the `create_lead` `qualification` schema that the Vapi path could not serialize, the actual cause of the empty-args misfires the Sonnet swap never fixed), resubmitted A2P with a verifiable `/sms-opt-in` opt-in page (now `IN_PROGRESS`), and finished the blog (all hero images unique). The two things to confirm next are a live voice call proving the lead fix books a real HCP estimate, and an A2P status re-check for approval. Full detail in the SESSION 31 block. Start here.

### Do these first (in order)
1. **One validating voice test call to confirm the lead-booking fix. THIS is the thing to verify.** Session 31 fixed the real root cause of the empty-args lead bug: `create_lead_with_estimate`'s `qualification` field was a `z.record` dynamic-key object that `z.toJSONSchema` renders with `additionalProperties`/`propertyNames`; the Vapi tool-calling path could not serialize that open-object, so the model emitted no args at all (50 of 52 voice bookings were empty `{}` and never reached HCP). Flattened it to a plain string. Place a real inbound call and run a fake estimate (name, callback number, service like "panel upgrade", address), then check `/switchboard/call-logs` + `/switchboard/lead-pipeline`: the lead must land as a real HCP estimate, NOT a "Lead capture did not complete" office flag. **If it STILL misfires (empty args), the schema was not the whole cause** — next lever is server-side lead reconstruction on misfire (build the HCP lead from the name+phone already saved by `update_visitor_contact` + the transcript, instead of relying on the model to emit the payload). Do NOT chase the model: prod voice IS on Sonnet 4.6 and that did not fix this.
2. **Re-check A2P for approval (resubmitted session 31, now `IN_PROGRESS`).** Session 31 deleted the FAILED campaign and re-POSTed a fresh one via the Twilio API with a `message_flow` that points to the new public `/sms-opt-in` page (which shows the consent CTA + verbatim language at one verifiable URL, the exact thing the 30909 reviewer could not find behind the multi-step `/quote` form). Re-check with the command in Key facts. **When it shows `REGISTERED`/`APPROVED`:** set `TWILIO_SMS_ENABLED=true` on Vercel `tz-electric` Production, redeploy, send a test page, confirm Messages status = `delivered` not `undelivered/30034`. If it FAILS again, pull the new error (it should no longer be CTA/30909).
3. **Watch the nightly "Lead-booking tool misfires" count after a day of the fix.** Session 31 confirmed it was NOT 0 — it was ~50/52 on Sonnet (the bug was never fixed despite the session-29 claim). After the schema flatten it should drop toward 0. That is the at-scale proof.
4. **Still on the test-call list (carried):** returning-customer recognition (call from David Kloss 845-853-2224, recognize the project by name, do not re-qualify, do not volunteer a date) and the Vapi "still working" line on slow tools.

### Blog follow-ups (optional polish, not blocking)
The blog is live at `/blog` with 20 migrated posts (footer + header links, redirects, sitemap). Easy follow-ups whenever wanted: (a) **publish dates are approximate**, the old Webflow site did not expose real dates so they are assigned in listing order Jan to Jun 2026, one-line-editable per post in the `date:` frontmatter; (b) ~~a few posts reuse the same hero image~~ **DONE session 31:** the 5 posts that shared 2 reused stock images now have unique, generated hero images (verified 0 duplicate hashes); (c) **Phase 2 is a Switchboard authoring UI** so Tyler can write and publish posts without code (like admin Claire for the KB), deferred, build if Tyler wants self-serve blogging. To publish a new post today: drop a `<slug>.md` into `src/content/blog/` plus a hero at `/public/images/blog/<slug>.webp`, then commit.

### Needs Tyler (blocked on his input — ask him)
- **"Speak to a human" behavior (NEW, session 29 deliberately left this for Tyler).** Claire's reports show she lost 4 leads on 6/2 because she drops to "the office will call you back" the instant a caller asks for a person, even mid-qualification on a real job. Her own proposed fix is to try ONCE more to book before taking a message. BUT the current hard-stop-on-human-request rule exists specifically to fix the Nick-call "agent agent agent" fight, so flipping it risks re-introducing that. One-line decision needed: when a caller asks for a human mid-qualification, keep trying to book once, or take a message and stop? (Rule lives in `agent-prompt.ts` voice block, the "Agent / person / human trigger words" section.)
- **Warehouse Associate posting specifics (NEW, session 29).** Live at `/careers/warehouse-associate` with defaults: pay `$20 – $28/hr (experience-based)`, M-F full-time. Confirm or correct the pay and schedule (one-line edit in `src/lib/careers-data.ts`).
...
### What shipped session 30 (live + verified — commit `36bf342`)
**The blog is live.** 20 posts migrated from the old Webflow blog (`tzelectricinc.webflow.io/blog`) into a file-based markdown blog: `/blog` index (featured + grid) and `/blog/[slug]` articles, statically generated with BlogPosting + breadcrumb JSON-LD, `.blog-prose` styling. Hero images pulled local to `/public/images/blog`. Blog link wired into the footer + the header About menu (desktop + mobile). 7 old duplicate/typo slugs 301-redirect to canonical posts. Index + all posts added to the sitemap. Also caught the A2P regression (FAILED, error 30909). Full detail in the SESSION 30 block below.

### What shipped session 29 (live + verified — commits `4458f9b`, `840ac51`, `d8296ee`)
**The voice lead funnel was silently 100% broken.** [**Session 31 correction:** the Haiku→Sonnet swap below did NOT fix lead booking. The model was never the cause; the real cause was the `create_lead` `qualification` `z.record` open-object schema the Vapi path cannot serialize, fixed in session 31. The reasoning-leak half of this was a real Sonnet win; the lead half was not. See the SESSION 31 block.] Voice model Haiku 4.5 to Sonnet 4.6 (the empty-args + reasoning-leak root cause; pinned in code). create_lead schema hardened (last name / service label / ownership optional with safe defaults so a partial lead still lands; structured qualification + misfire guard unchanged). Voice prompt rules: "never think out loud" (the Cindy fix), "Just a sec" added to the stall ban, ask-only-for-the-wrong-digits on a misheard number. Stripe plan-signup office email now shows the contract Term (3-year / annual / month-to-month), not just Monthly/Yearly. Nightly analyzer now inspects tool payloads and flags empty-args misfires (the blind spot that hid this bug for weeks). Warehouse Associate job posting added at `/careers/warehouse-associate`.
...
### Key facts / gotchas (carried forward; updated through session 31)
- **Blog is file-based, no DB, no migration.** Posts are markdown in `src/content/blog/<slug>.md` (frontmatter + body) rendered with `marked` via `src/lib/blog.ts`; `/blog` and `/blog/[slug]` are statically generated (`dynamicParams = false`). Article styling is a hand-rolled `.blog-prose` block in `globals.css` (the site has no `@tailwindcss/typography` plugin; the `prose` classes elsewhere are inert). Old-slug redirects live in `next.config.ts`. Publish a new post: drop a `<slug>.md` + a hero at `/public/images/blog/<slug>.webp`, commit.
- **Migrations at 019.** `npm run migrate` is idempotent (tracked in `_migrations`). 018 (call-review marks) + 019 (inbound caller phone) applied to prod. Session 30 added no migration (blog is file-based).
...
- **Voice model is now Sonnet 4.6 (DONE session 29), pinned in code** at `src/app/api/agents/voice/server/route.ts` (`MODEL_NAME = process.env.VAPI_MODEL_NAME || 'claude-sonnet-4-6'`). `VAPI_MODEL_NAME` was REMOVED from Vercel prod so the version-controlled code default governs (the CLI `env add` kept writing an empty value over a piped stdin; to pin a different model use the Vercel dashboard, not the CLI). Vapi's accepted Anthropic models (verified by creating + deleting a throwaway assistant): `claude-sonnet-4-6`, `claude-sonnet-4-5-20250929`, `claude-opus-4-6`, `claude-opus-4-5-20251101`, `claude-haiku-4-5-20251001`, plus older 3.x; bare `claude-sonnet-4-5` (no date) is rejected. Confirmed with hard proof: same shared tool, voice Haiku 0/41 args vs web-chat Sonnet 9/9. **CORRECTION (session 31): the model was NOT the root cause.** Prod runs Sonnet 4.6 and still misfired 50/52 voice bookings. The real cause was the `create_lead` `qualification` `z.record` open-object schema the Vapi tool-calling path cannot serialize, fixed by flattening it to a plain string. The Haiku-vs-Sonnet arg-fill gap was a red herring: web chat books via the AI SDK and voice books via Vapi (different tool-calling paths, not just different models).
...
- **Voice lead bug + fix (session 31).** `create_lead_with_estimate`'s `qualification` field is now a plain summary **string** (was `z.record`, a dynamic-key object the Vapi tool path could not serialize → empty `{}` args → 50/52 misfires). The impl (`agent-tools.ts`) coerces the string into `{summary}` for the office notes; web chat is unaffected. The empty-args misfire guard in `voice/server/route.ts` (`lacksCoreContact`) still routes a contactless misfire to one office flag. Confirm with a live call before assuming it holds.
- **A2P opt-in page.** `/sms-opt-in` is the public, verifiable opt-in CTA page (consent checkbox + verbatim language + STOP/HELP + no-third-party-sharing), linked from the footer + sitemap. The campaign `message_flow` references it. Resubmitted via the Twilio API in session 31 (deleted the FAILED campaign, re-POSTed); now `IN_PROGRESS`. Prior campaign backed up at `/tmp/a2p-backup-prev.json` (transient).
...
### ✅ SESSION 31 UPDATE (2026-06-10, Cesar, second sitting) — The REAL voice lead-bug fix + A2P resubmit + unique blog images (live, commit `bc92cec`)

Cesar came back with three asks: investigate a call Tyler thought Claire hung up on, fix + resubmit A2P, and make every blog hero image unique. A log dive turned up a much bigger finding than the hangup.

**The "hangup" call (number ending 2507) — Claire did NOT hang up.** Conv `86bda8ee`, Raffid (838-667-2507), 2026-06-10 5:31 PM. `closed_reason = customer-ended-call`: the CALLER hung up. He asked for a human in his first breath; Claire took his name + callback, fired `flag_for_office_review` (so the office WAS notified), said the team would call back, and he ended the call. What made it look bad: she drops straight to take-a-message on a human request (the open "speak to a human" decision for Tyler), misheard "Raffid" as "Rigg", and used filler. Across all 269 voice calls in 21 days she is NOT cutting off engaged callers: 223 caller-ended, 22 silence-timeouts (caller went quiet 30s), 17 "assistant-said-end-call-phrase" — and all 17 were zero-caller-turn dead-air/spam calls.

**The real find: voice lead booking was still ~100% broken; the session-29 "Sonnet fixed it" was wrong.** Of 52 `create_lead_with_estimate` calls in 21 days, **50 fired with empty `{}` args** (only Adam + Casino populated args and created an HCP estimate), including fully-qualified calls on 6/10 (Joy gave name, service, full Saugerties address, homeowner, last name across 12 turns, then the tool fired empty). Prod is confirmed on Sonnet 4.6 (`VAPI_MODEL_NAME` absent from Vercel), so the model was never the cause. Root cause: the tool's `qualification` field was `z.record(z.string(), z.string())`, a dynamic-key object `z.toJSONSchema` renders with `additionalProperties` + `propertyNames`; the Vapi tool-calling path could not serialize that open-object, so the model emitted no args at all. Web chat fills the same tool 9/9 because it books through the AI SDK, not Vapi. **Fix (commit `bc92cec`):** flattened `qualification` to a plain summary string in the shared Zod schema (`agent-tools.ts`); the impl coerces the string into `{summary}` for the office notes; web chat unaffected (a string is easier to fill than a map); the empty-args misfire guard still protects the worst case. **Needs one live call to confirm at scale** (Do these first #1). The filler ("Give me a moment", "This will just take a sec") is a separate issue: every phrase she used is ALREADY banned in `agent-prompt.ts` (lines ~170-176), so it is an adherence problem on the Vapi path, not a missing rule — no prompt change made.

**A2P resubmitted (now `IN_PROGRESS`).** The 30909 rejection was "CTA could not be verified": the SMS consent checkbox lives on the last step of the multi-step `/quote` form, so a carrier reviewer landing on `/quote` could not see it. Built a public `/sms-opt-in` page that shows the three opt-in methods, the verbatim consent checkbox + language, message types/frequency/rates, STOP/HELP, and the no-third-party-sharing clause at one verifiable URL; linked it from the footer + sitemap. Then deleted the FAILED campaign (a FAILED A2P campaign cannot be PATCHed) and re-POSTed a fresh one via the Twilio API, reusing every prior field and changing ONLY `message_flow` to reference `/sms-opt-in`. New status: `IN_PROGRESS`, no errors, `message_flow` confirmed referencing the page. Carrier review is now out of our hands; if it FAILS again the error should no longer be CTA/30909.

**Blog images — all unique now.** 5 posts shared 2 reused stock images from the old Webflow site (`1pic` ×3, `upgrade2` ×2). Generated 3 new context-matched images (outdated wiring; two distinct mini-split interiors) via the Higgsfield image tool, replaced the duplicates, verified 0 duplicate hashes remain. `/blog` is fully done.

No migration. SSD = Dropbox = origin.

---

### ✅ SESSION 30 UPDATE (2026-06-10, Cesar) — Blog migration + A2P regression finding (blog live + verified, commit `36bf342`)

Two things this session: shipped the blog, and caught that A2P had silently regressed.

**Blog (shipped + live).** The old Webflow blog (`tzelectricinc.webflow.io/blog`, 26 URLs) was migrated into a new **file-based** blog on the Next.js site.

- **Architecture.** Posts are markdown files in `src/content/blog/<slug>.md` (frontmatter: title, description, category, date, heroImage, sourceImage, author; then the body). `src/lib/blog.ts` reads them at build time, parses the frontmatter (small custom parser, no new dep), and renders the body with `marked` (already a dependency). `/blog` (index: featured post + responsive grid) and `/blog/[slug]` (article) are statically generated with `dynamicParams = false`, each with BlogPosting + breadcrumb JSON-LD. Article styling is a hand-rolled `.blog-prose` block in `globals.css` (the site has no `@tailwindcss/typography` plugin, so the `prose` classes used elsewhere are inert; matched the existing `.kb-prose` approach instead).
- **20 unique posts.** The 26 old URLs included 6 duplicate-topic slugs (an older Webflow batch) plus 1 typo slug (`lighing`). Those 7 now 301/308-redirect to their canonical posts via `next.config.ts`. Content was scraped with Firecrawl (4 parallel subagents), cleaned (breadcrumbs, duplicate H1, testimonials/financing/footer boilerplate stripped; internal links rewritten to relative paths), and house-style dedashed (37 em/en-dashes converted to ASCII punctuation across 7 files). Faithful migration, no rewrites.
- **Hero images local.** All 20 heroes downloaded to `/public/images/blog/<slug>.webp` so the new site does not depend on the old Webflow CDN. One post's source was a 4.7 KB thumbnail (`.avif`), swapped to the existing `/images/services/mini-split.webp`.
- **Nav.** Blog link enabled in the footer (`footerCompany` in `Footer.tsx`) and the header About menu (`ABOUT_LINKS` in `Header.tsx`, which feeds both the desktop mega-panel and the mobile accordion). Both files had a pre-existing commented-out Blog slot from the original build.
- **Sitemap.** `/blog` + all 20 posts added to `sitemap.ts` (imports `getAllPosts`).
- **Verified live.** `tsc` + `next build` clean (21 blog routes prerendered). After deploy: `/blog` 200, posts 200, old-slug redirects 308 to canonical, index renders post titles, article H1/body render, footer Blog link present.
- **Caveats / follow-ups** (see NEXT SESSION → Blog follow-ups): publish dates are approximate (old site did not expose them; assigned in listing order, editable per post); a few posts reuse the same hero image (the old site did too); a Switchboard authoring UI for Tyler (Phase 2) is deferred.
- **Publish a new post:** drop a `<slug>.md` into `src/content/blog/` + a hero at `/public/images/blog/<slug>.webp`, commit.

**A2P regression caught.** The handoff recorded the SMS campaign as `IN_PROGRESS` (last seen by Dennis 2026-05-29). A live status pull this session shows it is actually **`FAILED`** (dated 2026-05-29), rejected with **error 30909** ("the campaign submission has been reviewed and rejected due to issues verifying the Call to Action (CTA)") on `MESSAGE_FLOW`. So Dennis's 5/29 API resubmission was reviewed and rejected the same day, a different failure mode than the original 30896/30882. SMS stays carrier-blocked (error 30034 on send) until this is fixed and re-approved. Fix path in NEXT SESSION → Do these first #1. No code change this session for A2P (it is a Twilio-side compliance fix).

No migration this session (the blog is file-based). 10 Claire tools unchanged. SSD = Dropbox = origin after the commit (the post-commit hook auto-pushed to GitHub and fast-forwarded the Dropbox mirror).

---

### ✅ SESSION 29 UPDATE (2026-06-03, Cesar) — Claire checkup: the empty-args lead bug, Sonnet 4.6, + tooling (live + verified, commits `4458f9b` → `d8296ee`)

A full read of the Neon logs (call logs, web chat, every nightly report, and tool payloads) surfaced one root cause behind a cluster of symptoms, plus two follow-on improvements and Tyler's warehouse request.

**The headline find: voice booked 0 leads into HCP, and had since launch.** `create_lead_with_estimate` fired with empty `{}` args on 41 of 41 voice calls over 9 days. The session-27 misfire guard (`da9f7ef`) caught each one and dropped it to a manual `flag_for_office_review`, which made it look handled (the office got a bare callback) but no HCP estimate was ever created and the qualification Claire gathered never reached HCP. Claire's own nightly reports saw the symptom ("0 leads booked") but not the cause, because the analyzer read transcripts, not tool payloads. **Proof it was the model, not the schema or the Vapi integration:** the exact same shared Zod tool, called from web chat (Sonnet 4.6 via AI Gateway), populated args 9/9; from voice (Haiku 4.5) it was 0/41. Smaller voice tools worked fine under Haiku (`update_visitor_contact` 60/60, `flag_for_office_review` 56/56). Haiku could not assemble the one large 6-field payload. The same root cause produced the **Cindy reasoning leak** (conv `d22ddb71`, 2026-06-03): Haiku spoke its chain-of-thought aloud to the caller ("the customer said next month... let me clarify... that's interesting, it could mean...").

What shipped (all `tsc` + `next build` clean, deployed, prod health-checked):

- **`4458f9b` — Voice model Haiku 4.5 to Sonnet 4.6.** Pinned as the code default in `voice/server/route.ts`; `VAPI_MODEL_NAME` removed from Vercel prod. Vapi-verified it accepts `claude-sonnet-4-6`. The real fix for the empty-args bug and the reasoning leak, and it lifts the chronic stall/filler/"technician" adherence misses too. Tradeoff: higher per-call token cost + slightly more latency than Haiku, accepted because voice was booking 0 leads. **Cesar's domain (model + prompt). Pending one live test call for end-to-end behavioral confirmation.**
- **`4458f9b` — create_lead_with_estimate schema hardened.** `last_name`, `service_label`, `ownership` are now optional with safe defaults (service label derives from `service_key`), so a real-but-partial lead lands instead of bouncing on Zod. The structured `qualification` map (safety tags ACTIVE LEAK, Medical Equipment) is unchanged, and the misfire guard is unchanged. Web chat (which already filled the full schema) is unaffected. Still 10 Claire tools.
- **`4458f9b` — voice prompt rules** in `agent-prompt.ts`: a "Never think out loud" block (speak only the reply, no third-person narration of the caller, no "let me clarify" deliberation, grounded in the Cindy call), "Just a sec" added to the forbidden stall list, and ask-only-for-the-wrong-digits when a caller corrects a misheard number.
- **`4458f9b` — Stripe plan-signup office email** (`agent-notifications.ts` + `housecall-pro.ts`): now renders the contract **Term** (3-year agreement / annual / month-to-month) from `planInfo.frequency`, not just the Monthly/Yearly billing cadence. Tyler's 6/3 ask (he could see a signup but not that it was a multi-year contract). `LEAD_FORM_TO_EMAILS` still controls recipients.
- **`840ac51` — nightly analyzer now inspects tool payloads** (`claire-self-improvement.ts` + the report renderer in `agent-notifications.ts`). Deterministic `extras.lead_tool_misfires` counts empty-args calls to args-required tools (read-only lookups excluded); the LLM transcript marks "⚠ EMPTY ARGS" on a misfire and the system prompt flags any nonzero count as HIGH severity (the one case where N=1 is surfaced); the daily email shows the count in red when > 0. This class of bug now self-reports the next morning instead of needing a manual log dive.
- **`d8296ee` — Warehouse Associate job posting** at `/careers/warehouse-associate` (`careers-data.ts`). Auto-renders with JobPosting schema + metadata, listed on `/careers`, applies via the existing shared Typeform. Pay/schedule are defaults pending Tyler (see Needs Tyler).

**Deliberately NOT shipped:** the "speak to a human" behavior change (see Needs Tyler) and the warehouse pay/schedule confirmation. Sync verified after each commit: SSD = Dropbox = origin (the post-commit hook auto-pushes to GitHub and fast-forwards the Dropbox mirror; Dennis's drive lags until he pulls).
...
- **Issue 1 — Claire re-qualified an existing customer (felt like re-selling).** David called to follow up on a 100-to-200 amp service upgrade already booked in HCP, and Claire ran the full new-lead qualification battery (project type, overhead/underground, reason, timeline) as if he were new. Root cause: the existing-customer signal (she even matched him via `find_existing_customer`) didn't change her flow, and she had no view of his existing estimate/job. Fix = **A + B**:
  ...
  - **A (prompt rule)** in `agent-prompt.ts`: new "Returning Customer Following Up On Existing Work" section + a Tool-Use bullet. On follow-up signals (or an HCP match), Claire skips qualification, confirms the existing project by name ("is that the one you're calling about?"), takes a callback number, and routes to the office via `flag_for_office_review` ("Existing customer follow-up:"). Cesar's domain.
  - **Refinement (Tyler, 2026-06-03, commit `2d48f7c`):** `lookup_existing_project` returns project NAMES + status only — **no dates or scheduled times** (the upcoming job contributes its description, not its schedule). The prompt forbids volunteering scheduled dates / "you're on the calendar" unless the caller specifically asks about timing; that path stays on `lookup_my_appointment`. So Claire recognizes the project to avoid re-qualifying, without committing to a date the office may still be finalizing.
...
Noted for later (DONE in session 29): **model swap Haiku → Sonnet** as the real voice-adherence lever — most failures were the model not following existing rules. Session 29 confirmed this and swapped to Sonnet 4.6.
...
- **#5 Dispatch visibility page + Twilio delivery tracking — ✅ SHIPPED + VERIFIED (commit `2eb45d1`).** Closes the "after-hours dispatch is a black box" gap. Layer 1 (additive data): `twilio-outbound.ts` `sendSms`/`placeCall` now pass a `StatusCallback` (send behavior UNCHANGED — Twilio just reports back); migration `015` adds `error_code` to `tz_dispatch_attempts` (twilio_sid/delivered_at/status already existed); new `/api/webhooks/twilio-delivery` validates `X-Twilio-Signature`, matches `MessageSid`/`CallSid` to an attempt, writes back status (delivered/failed) + delivered_at + error_code (only UPDATEs matched rows — never creates/deletes). Layer 2 (view): new owner/admin-gated `/switchboard/after-hours` page — one **collapsible card** per `tz_emergency_dispatches` (click to expand the attempt ladder; active/open cards default expanded, resolved start collapsed; collapsed header shows a delivered/failed/sent roll-up) with real per-attempt delivery status (green delivered / red failed + code); registered in `modules.ts` + `nav-config.ts`. Card UI is the client component `AfterHoursClient.tsx` (commit `58f2c21`); page does the server-side fetch + auth. Verified in prod: webhook unsigned POST → 403, page unauth → 307 redirect, queries render the real Louis Armistead 13-attempt dispatch. Note: existing (pre-deploy) attempts still show `sent` since they predate the callback; NEW dispatches get real delivered/failed. While SMS is A2P-blocked, dispatch SMS attempts will show `failed (30034)` and voice will show `delivered` — exactly the visibility Tyler wanted.

- **#6 User management — inline name edit + Disable/Re-enable — ✅ SHIPPED + VERIFIED (commit `f06a466`).** Owner-only. New `setUserName` (users.ts, audit-logged `user.name_changed`) + `/api/switchboard/users/name`. In `UsersClient.tsx`: click the ✎ next to a name to edit inline (Enter saves / Esc cancels); explicit **Disable** (red, with confirm) on active non-owner rows and **Re-enable** (green) on disabled rows, reusing the proven `setUserRole` path (Disable → `disabled` stamps `disabled_at` + blocks access via `canAccessSwitchboard`; Re-enable → `office`). Soft-disable preserves account + audit/attribution (no hard delete). Role dropdown untouched; can't disable self or an owner. Verified: page 307, name API 401 unauth, Dennis confirmed name edit works in the UI.

- **Claire 5/28 self-improvement report — implementation audit + proper-noun spelling rule (session 26).** Reviewed Cesar's 5/28 analyzer report against the live prompt/KB. Finding: most proposed items were ALREADY implemented (frustration/"want a person" handoff, no-unprompted-pricing, stall-phrase ban, estimator-vs-tech, EV customer-supplied + permits-not-included KB policies). Genuine gaps: (1) proper-noun spelling, (2) permit/COI-specific KB handling, (3) three open "Questions for Tyler". **Implemented gap #1** (`agent-prompt.ts`, commit below): extended the names/spelling block to cover **town + street names** with a "NEVER confirm back a guessed spelling — ask to spell the FIRST time" rule and a local Hudson Valley town list (Catskill, Saugerties, Shokan, etc.) so Claire stops producing "Socrates"/"Nyehan"/"Sho". Still TODO: permit/COI KB entry (needs Tyler's turnaround/detail preference) + Tyler's 3 open questions (notify_team_member-for-named-callbacks policy, COI detail level, EV supply policy). NOTE: prompt change — Cesar's domain; made at Dennis's direction.

- **#7a — "accept No without re-asking" prompt rule (session 26).** Added a rule in `agent-prompt.ts` (next to "Accept I Don't Know"): when a caller answers "no" to a question/confirmation, acknowledge briefly and move on — never re-ask reworded or press for a reason. Fix to the Joey-call pattern (re-asking frustrates callers into hanging up).
- **#7b — opener A/B test (session 26).** Voice route (`/api/agents/voice/server`) now serves one of two openers at random per call: `OPENER_A` (current full greeting) vs `OPENER_B` (shorter: "Hi, thanks for calling TZ Electric. This is Claire, your smart assistant — what can I do for you?"). Both keep the required "smart assistant" disclosure. **Measurement is free** — the opener is captured as the first assistant message in each transcript, so compare early-hang-up rates by opener text (the nightly analyzer or a simple query). To end the test, set both constants to the winner. Goal: cut the ~8 opener-hang-ups the session-24 analyzer flagged.

- **#7c — `lookup_my_appointment` tool (calendar read) — BUILT + TESTED (session 26).** Read-only tool so Claire can answer "what time is my appointment?". New `getUpcomingJobForCustomer(hcpCustomerId)` in `housecall-pro.ts` (GET /jobs?customer_id=, filters to soonest FUTURE non-canceled scheduled job). New `lookup_my_appointment` tool + `lookupMyAppointmentImpl` in `agent-tools.ts`: identifies the caller by the conversation's phone → `tz_hcp_customers` mirror, requires an UNAMBIGUOUS single match, returns only the soonest upcoming job; declines (found=false) on no-number / no-match / multiple-matches so it never leaks the wrong person's schedule or any history. **Purely additive** — registered alongside existing tools (auto-wires to voice/web/SMS via `buildVapiFunctionDefinitions`); no existing tool or prompt rule changed/removed. Verified against live HCP+mirror data (returned a real June-12 job; ambiguity + decline paths work). **NOT YET "taught" in the prompt** — per Dennis, the tool exists and is callable (the tool description guides the model), but we deferred adding an explicit prompt rule until we confirm she invokes it well; add a small prompt rule (Cesar's domain) if needed.

- **#7f — tool retry-cap prompt rule (session 26).** Investigation first (read-only): conv `0717f9c8` fired `create_lead_with_estimate` 13× with EMPTY args — all rejected by the Zod validation guard, so **no duplicate records were created** (`tz_leads` for that phone = 0). The issue was just a wasteful retry loop. Fix: added a "Tool errors — never loop" bullet to the shared tool-guidance in `agent-prompt.ts` — on a validation/other tool error, fix the args and retry at most ONCE, then `flag_for_office_review` and move on; never call the same tool more than twice in a row. Additive, prompt-only (Cesar's domain). Did NOT do the bigger code-side option (auto-fill create_lead args from saved contact) — touches the production lead path, left as a noted future hardening.

- **#3 phase-2 + 7c "teach" (session 26).** (a) **Web/SMS name recognition** — extended the HCP customer match beyond voice: `update_visitor_contact` (the shared phone-capture tool for web chat + SMS) now silently links `hcp_customer_id` to the conversation when the captured phone matches the synced mirror (`COALESCE` — never overwrites, non-fatal, no behavior change). Mirrors what the voice path does at call start. (b) **Taught Claire the appointment tool** — added a `lookup_my_appointment` bullet to the tool guidance so she reaches for it on "what time is my appointment?" and declines gracefully (take a message) when found=false. Both additive, type-checked.

- **Voice interruption (barge-in) tuning (session 26).** Tyler/Dennis noticed Claire stopped talking on any little sound (cough / background blip / line static) and then went silent — Vapi's default `stopSpeakingPlan` interrupts on ANY audio (numWords 0). Added to the voice assistant config (`/api/agents/voice/server`): `stopSpeakingPlan { numWords: 1, voiceSeconds: 0.3, backoffSeconds: 1.0 }` + `backgroundDenoisingEnabled: true`. Now a sub-word sound won't stop her, but a real word will. Dennis chose numWords 1 (vs 2). Tune up to 2 if still twitchy on noisy lines. Additive config only — no prompt/logic change.

- **#7d — Central Hudson utility routing (session 26).** Two triggers skip the homeowner qualification flow and route to the office (`flag_for_office_review`, reason "Central Hudson utility call:"): (1) inbound caller ID is a known Central Hudson number — `CENTRAL_HUDSON_NUMBERS` (`8454522000`, `8454522010`) in the voice route → passes `utilityCaller: true` to `buildSystemPrompt` which injects the skip-qualification instruction; (2) the caller SAYS they're from Central Hudson / a utility (general prompt rule, any channel). **Claire never proactively asks** anyone if they're from a utility (Dennis 2026-05-29). Additive: new optional `utilityCaller` prompt input (defaults off → no change for normal callers), new prompt section, new number constant. Routing is to the office for now; live transfer to a named staffer waits on #4. Add more utility numbers to the constant as needed.

- **NEW (Cesar greenlit, session 26) — Follow-Ups / Callbacks hub — SHIPPED + TESTED.** A live Switchboard page (`/switchboard/follow-ups`, owner/admin/office) listing every open callback Claire flagged (`notify_team_member` + `flag_for_office_review`) over the last 14 days, grouped by who it's for (+ a General office bucket), with customer, click-to-call number, Claire's note, time, a deep-link to the conversation, and a **Mark-done dropdown that records the OUTCOME** (Reached–booked / Reached–declined / No answer / Left message / Wrong number / Other-with-note). Closed items move to a collapsible **"Recently handled (last 7 days)"** section showing the outcome + who closed it + when (reopenable) — so the office can prove the callback was actually made. Outcome enhancement: migration `017` adds `outcome` to `tz_followup_resolutions`; outcomes are a shared pure constant `src/lib/followup-outcomes.ts` (client labels + server validation); `getRecentlyHandled()` feeds the closed section. It's the live, always-on version of the 6 PM recap email — anyone with access sees who still needs a callback without digging through email; directly closes the "no real-time view of who to call back" gap. Pieces: migration `016` (`tz_followup_resolutions`, keyed on the flag's message id — presence = done; additive, FK cascade), `src/lib/followups.ts` (open query + resolve/reopen), `/api/switchboard/follow-ups/resolve` (owner/admin/office), `/switchboard/follow-ups` page + `FollowUpsClient.tsx`, registered in `modules.ts` + `nav-config.ts`. **Additive** — never touches Claire, the flag tools, or the EOD recap. Tested end-to-end on real data (24 open items; resolve 24→23, reopen →24, no residue).

**Still open from the session-25 list:** #4 Claire call transfer — **BLOCKED: need the exact extra HCP voicemail number from Tyler before building** (Dennis confirmed 2026-05-29); #7 smaller items (see that section — CRON_SECRET sub-item already done this session).

**Session 26 commits (in order):** A2P resubmit was done via the Twilio API (no commit). Code/docs: `8824b86` (HCP name sync) → `c805606` (docs) → `3cceb5f` (EOD recap build) → `ced39a6` (docs) → `728ad35` (EOD 6 PM ET DST-proof) → `c69f81c` (bind CRON_SECRET) → `d88547b` (docs: CRON_SECRET done) → `3020a34` (voice Claire message/voicemail tool-firing fix) → `7b49656` (docs sync) → `2eb45d1` (dispatch visibility + Twilio delivery tracking) → `b049ac4` (docs) → `58f2c21` (after-hours collapsible cards) → `1651da2` (docs) → `f06a466` (user mgmt name edit + disable). All deployed to prod via the auto-deploy hook and verified live.
...
Voice cost baseline (last 14 days, from Vapi): **$7.88 / 79 calls / ~$0.11 per minute.** Breakdown: Vapi platform fee 46%, Claude Haiku LLM 24% (this baseline is Haiku-era; session 29 swapped voice to Sonnet 4.6, so expect the LLM share + per-minute cost to rise. Worth it: voice was booking 0 leads on Haiku. Monitor `tz_voice_call_costs` and `/switchboard/cost-analysis`), post-call analysis 20% (now disabled), Deepgram STT 9%, 11labs $0 on Vapi (BYOK, billed separately ~$8-13). Anthropic prompt caching is NOT available through Vapi (their `AnthropicModel.messages` is bare `OpenAIMessage`, no `cache_control` passthrough) — would require a custom-LLM-URL proxy, deprioritized. Cesar turned on Vapi auto-reload so the wallet never zeroes out again.
...
- **Voice Claire:** dial **+15186786153** (Hudson Valley Twilio number on Tyler's account, voice via Vapi BYON, **Claude Sonnet 4.6** + 11labs Eryn voice (model swapped from Haiku 4.5 in session 29), full TZ knowledge base injected per call via dynamic server-URL pattern, all 10 Claire tools wired)
...
| `VAPI_MODEL_NAME` | no (removed session 29) | Optional voice model override. REMOVED from Vercel prod in session 29 so the code default `claude-sonnet-4-6` governs (set it via the Vercel dashboard, not the CLI, if you ever need to pin a different model). Web chat uses Sonnet 4.6 via the AI Gateway. |

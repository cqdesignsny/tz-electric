...
1. **One validating voice test call** — a batch of live voice changes from sessions 27–28 can only be confirmed on a real call:
   - **Session 28 — returning-customer recognition.** Call from a number that maps to an HCP customer with an open estimate/job and ask a follow-up ("when are you coming to do my upgrade?"). Claire should call `lookup_existing_project`, recognize + confirm the project BY NAME ("is that the one you're calling about?"), and NOT re-run new-lead qualification — and per Tyler (2026-06-03) she should NOT volunteer the appointment date/time unless the caller specifically asks (timing comes from `lookup_my_appointment`). Live target: **David Kloss's number (845-853-2224) has a real "100-200amp Overhead Service Upgrade" estimate + a June 11 job**, so it resolves end-to-end.

...
**Last verified:** 2026-06-03, **session 29** (Cesar). Session 29 swapped Voice Claire from Haiku 4.5 to Sonnet 4.6 (fixing a silent empty-args bug that had voice booking 0 HCP leads, plus a reasoning leak), hardened the lead tool, taught the nightly analyzer to catch tool misfires, surfaced the Stripe contract term, and added a Warehouse Associate job posting. All live in prod (HEAD `d8296ee`; SSD, Dropbox, and origin all in sync). See **"⭐ NEXT SESSION — START HERE"** directly below, then the **SESSION 29 / 28 / 27** detail blocks. All prior history (session 26 and earlier) is preserved underneath.

---

## ⭐ NEXT SESSION — START HERE (picking up after session 29, 2026-06-03)

Session 29 is live in prod and pushed (HEAD `d8296ee`; SSD = Dropbox = origin). It was a Claire diagnosis-and-fix session driven by reading the Neon logs: Voice Claire was on Haiku 4.5, which fired `create_lead_with_estimate` with empty `{}` args on every single voice call (0 leads ever booked into HCP from voice, masked by the session-27 misfire guard routing each to a manual office flag) and narrated its reasoning out loud (the Cindy call). The fix was the model: Haiku 4.5 to Sonnet 4.6. Full detail in the SESSION 29 block below. Start here.

### Do these first (in order)
1. **One validating voice test call to confirm the Sonnet 4.6 swap. THIS is the thing to verify.** Place a real inbound call and run a fake estimate: give a name, a callback number, a service ("panel upgrade"), and an address. Then check `/switchboard/call-logs` and `/switchboard/lead-pipeline`: the lead must now land as a real HCP estimate, NOT a "Lead capture did not complete" office flag. That proves the empty-args bug is fixed. Also confirm Claire does NOT narrate her reasoning ("let me clarify", "the customer said") and sounds natural with no stacked "just a sec" filler. **If anything is off, revert the model: re-add `VAPI_MODEL_NAME=claude-haiku-4-5-20251001` in the Vercel dashboard (CLI stdin piping is flaky for env values) and redeploy.** Still on the test-call list from 27-28: returning-customer recognition (call from David Kloss 845-853-2224, she should recognize the service-upgrade project by name, not re-qualify, not volunteer a date) and the Vapi "still working" line on slow tools.
2. **Read the first at-scale Sonnet report.** The 2026-06-04 nightly self-improvement report (fires 2 AM ET on 6/5, emailed and in `/switchboard/agent-training`) is the first full day of Sonnet voice data. It now has a "Lead-booking tool misfires" line near the top: it should read **0** (rendered red if not). That is the at-scale proof the swap held.
3. **Re-check A2P, then flip SMS if approved.** Unchanged since session 26 (last seen `IN_PROGRESS` 2026-05-29). Re-check via the command in Key facts. **If `REGISTERED`/`APPROVED`:** set `TWILIO_SMS_ENABLED=true` on Vercel `tz-electric` Production, redeploy, send a test page, confirm Messages status = `delivered` not `undelivered/30034`.
...
### Key facts / gotchas (carried forward; updated through session 29)

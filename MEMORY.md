...
- [ ] **2 remaining agent blockers** (section 10 of `docs/agent-training-answers.md`): drain cleaning rate, review platform scope. Other historic blockers resolved.
- [ ] **Voice agent (Claire) — NEXT.** Vapi + Twilio, 15-min max before forced handoff. Same `agent-prompt.ts` + `agent-tools.ts` as web chat. Tyler signs up at vapi.ai. ~2-3 hours wire-up.
- [ ] **SMS agent (Claire) — long pole.** Scaffolded; awaiting Twilio A2P 10DLC carrier review (1-2 weeks). Mirror the web-chat route's wiring (gateway() wrapper, prompt caching, abuse guardrails) when wiring the model call.
- [ ] **Reports module** at `/switchboard/reports` — Phase R1 charts off `tz_leads`, ~2 hours.
- [ ] **Phase 7 self-improving learning loop** — transcript flagging → approved KB overrides via existing override mechanism.
- [ ] **Email Assistant, Office Ops, Warehouse, Sales/Outbound modules** — info pages live, build out as ops grows.
